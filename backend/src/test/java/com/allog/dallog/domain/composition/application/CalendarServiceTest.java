...
class CalendarServiceTest extends ServiceTest {
    ...
    void 시작일시와_종료일시로_유저의_캘린더를_일정_유형에_따라_분류하고_정렬하여_반환한다() {
        // given
        authService.generateToken(MEMBER_인증_코드_토큰_요청());
        Member member = memberService.getByEmail(MEMBER_이메일);
        ...
        subscriptionService.save(member.getId(), BE_일정.getId());

        /* 장기간 일정 */
        scheduleService.save(member.getId(), BE_일정.getId(),
        ...
        CategoryResponse 우아한테크코스_외부_일정_응답 = categoryService.save(member.getId(), 우아한테크코스_외부_일정_생성_요청);
        ...
        subscriptionService.save(member.getId(), 우아한테크코스.getId());

        // when
        MemberScheduleResponses memberScheduleResponses = calendarService.findSchedulesByMemberId(member.getId(),
